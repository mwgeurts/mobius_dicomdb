        ...
        if ~isempty(regexpi(dicom{1}.files{j}, 'RTPLAN'))
            ...
            for k = 1:length(json)
                ...
                if isempty(c) && strcmp(json{k}.RTPlanName, d.RTPlanName)
                    ...
                    % Log event
                    Event(sprintf('Inserting patient %s plan %s into database', ...
                        json{k}.PatientName, json{k}.RTPlanName));
    ...
    for j = 1:length(dicom{1}.files)
        ...
        if ~isempty(regexpi(dicom{1}.files{j}, 'RTDOSE'))
            ...
            for k = 1:length(rtplans)
                ...
                if ~isempty(rtplans{k}) && ...
                        isfield(d, 'ReferencedRTPlanSequence') && ...
    ...
    for j = 1:length(dicom{1}.files)
        ...
        if ~isempty(regexpi(dicom{1}.files{j}, 'RTSTRUCT'))
            ...
            for k = 1:length(rtplans)
                ...
                if ~isempty(rtplans{k}) && ...
                        isfield(rtplans{k}, 'ReferencedStructureSetSequence') && ...
    ...
    for j = 1:length(dicom{1}.files)
        ...
        if ~isempty(regexpi(dicom{1}.files{j}, '/CT'))
            ...
            for k = 1:length(rtstructs)
                ...
                if ~isempty(rtstructs{k}) && isfield(rtstructs{k}, ...
                        ...
                        isfield(rtstructs{k}.ReferencedFrameOfReferenceSequence...
                        .Item_1.RTReferencedStudySequence.Item_1, ...
                        'RTReferencedSeriesSequence') && strcmp(...
                        rtstructs{k}.ReferencedFrameOfReferenceSequence...
                        .Item_1.RTReferencedStudySequence.Item_1...
